...
export default function PortfolioPage({
  ...
  setIsChatOpen,
  isDarkMode,
  apiBase, // <--- Receive the dynamic API URL from App.jsx
}) {
  ...
  // --- UPDATED FETCH WITH API_BASE ---
  const handleOpenDocFromChat = async (docTitle) => {
    try {
      const response = await fetch(`${apiBase}/files`); // <--- Dynamic URL
      ...
      const match = allFiles.find((f) => {
        ...
        const f_id = f.document_id.toLowerCase();
        return (
          fname.includes(cleanTitle) ||
          cleanTitle.includes(fname) ||
          f_id.includes(cleanTitle)
        );
      });
      ...
      if (match) {
        setChatPreviewDoc({
          ...
          size: match.size || "N/A",
          isLocal: false,
          // Help the modal find the file on the backend
          previewUrl: `${apiBase}/preview/${match.document_id}`,
        });
      }
    } catch (err) {
  ...
  // --- UPDATED ASSET CREATION WITH API_BASE ---
  const handleConfirmAdd = async (newAssetData) => {
    try {
      ...
      if (result.status === "success") {
        const refreshRes = await fetch(`${apiBase}/portfolio`); // <--- Dynamic URL
        const updatedData = await refreshRes.json();
        setPortfolioData(updatedData);
        setIsAddModalOpen(false);

        // Find the newly created asset to navigate to it
        const newAsset = updatedData.assets.find(
          (a) => a.folder_name === result.folder_name
        );
        if (newAsset) navigate(`/portfolio/${newAsset.id}`);
      }
    } catch (error) {
  ...
  // ... (handleFavorite and handleArchiveStatus logic remains same)
  ...
  return (
    ...
    >
      ...
      {/* Passing apiBase down to the ChatDrawer as well */}
      <ChatDrawer
        isOpen={isChatOpen}
